...
class sail_cSim(pluginTemplate):
    ...
    def runTests(self, testList, cgf_file=None, header_file= None):
        ...
        try:
            ...
            sail_config = json.loads('\n'.join([line for line in sail_config.stdout.splitlines() if not line.strip().startswith("//")]))
        except subprocess.CalledProcessError as e:
            print("sail_riscv_sim --print-default-config failed:", e.stderr)
            exit(1)
        except json.JSONDecodeError as e:
            print(f"sail_riscv_sim --print-default-config output is not valid JSON: {e}")
            exit(1)
