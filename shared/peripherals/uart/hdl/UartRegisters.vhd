...
entity UartRegisters is
    port (
        ...
        hwif_in : in UartRegisters_in_t;
        hwif_out : out UartRegisters_out_t
    );
end entity UartRegisters;

architecture rtl of UartRegisters is
    ----------------------------------------------------------------------------
    ...
    type \UartRegisters.tx.tx_combo_t\ is record
        next_q : std_logic_vector(7 downto 0);
        load_next : std_logic;
    end record;

    type \UartRegisters.tx_combo_t\ is record
    ...
    type field_combo_t is record
        tx : \UartRegisters.tx_combo_t\;
        ctrl : \UartRegisters.ctrl_combo_t\;
        status : \UartRegisters.status_combo_t\;
    end record;
    signal field_combo : field_combo_t;

    -- Field Storage Signals
    type \UartRegisters.tx.tx_storage_t\ is record
        value : std_logic_vector(7 downto 0);
    end record;

    type \UartRegisters.tx_storage_t\ is record
    ...
    type field_storage_t is record
        tx : \UartRegisters.tx_storage_t\;
        ctrl : \UartRegisters.ctrl_storage_t\;
        status : \UartRegisters.status_storage_t\;
    end record;
...
begin
    ...
    cpuif_generics: process begin
        assert_bad_addr_width: assert s_axil_i.ARADDR'length >= UARTREGISTERS_MIN_ADDR_WIDTH
            report "Interface address width of " & integer'image(s_axil_i.ARADDR'length) & " is too small. Shall be at least " & integer'image(UARTREGISTERS_MIN_ADDR_WIDTH) & " bits"
            severity failure;
        assert_bad_data_width: assert s_axil_i.WDATA'length = UARTREGISTERS_DATA_WIDTH
            report "Interface data width of " & integer'image(s_axil_i.WDATA'length) & " is incorrect. Shall be " & integer'image(UARTREGISTERS_DATA_WIDTH) & " bits"
            severity failure;
        wait;
    end process;
    -- pragma translate_on
    ...
    begin
        ...
        is_invalid_rw := '0';
        decoded_reg_strb.rx <= cpuif_req_masked and (cpuif_addr = 16#0#) and not cpuif_req_is_wr;
        decoded_reg_strb.tx <= cpuif_req_masked and (cpuif_addr = 16#4#) and cpuif_req_is_wr;
        decoded_reg_strb.ctrl <= cpuif_req_masked and (cpuif_addr = 16#8#);
        decoded_reg_strb.status <= cpuif_req_masked and (cpuif_addr = 16#C#) and not cpuif_req_is_wr;
    ...
    -- Field logic
    ----------------------------------------------------------------------------
    hwif_out.rx.rx.swacc <= decoded_reg_strb.rx;

    -- Field: UartRegisters.tx.tx
    process(all)
        variable next_c: std_logic_vector(7 downto 0);
        variable load_next_c: std_logic;
    begin
        next_c := field_storage.tx.tx.value;
        load_next_c := '0';
        if decoded_reg_strb.tx and decoded_req_is_wr then -- SW write
            next_c := (field_storage.tx.tx.value and not decoded_wr_biten(7 downto 0)) or (decoded_wr_data(7 downto 0) and decoded_wr_biten(7 downto 0));
            load_next_c := '1';
        end if;
        field_combo.tx.tx.next_q <= next_c;
        field_combo.tx.tx.load_next <= load_next_c;
    end process;
    process(clk) begin
        if false then -- async reset
            field_storage.tx.tx.value <= 8x"0";
        elsif rising_edge(clk) then
            if rst then -- sync reset
                field_storage.tx.tx.value <= 8x"0";
            else
                if field_combo.tx.tx.load_next then
                    field_storage.tx.tx.value <= field_combo.tx.tx.next_q;
                end if;
            end if;
        end if;
    end process;
    ...
    process(all)
        ...
        variable load_next_c: std_logic;
    begin
        next_c := field_storage.status.rxr.value;
        load_next_c := '0';
        
        -- HW Write
        next_c := hwif_in.status.rxr.next_q;
        load_next_c := '1';
        field_combo.status.rxr.next_q <= next_c;
